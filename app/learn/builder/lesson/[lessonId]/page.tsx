...
const XP_KEY = 'stockly_builder_xp'
...
export default function BuilderLessonPage() {
  ...
  useEffect(() => {
    ...
    for (const unit of BUILDER_UNITS) {
  ...
  function handleComplete(xp: number) {
    ...
    updateUser({ onboardingStep: '/learn/builder' })
    router.push('/learn/builder')
  }
  ...
  if (notFound) {
    return (
      <div className="max-w-sm mx-auto px-6 py-20 text-center">
        ...
        <button onClick={() => router.push('/learn/builder')} className="text-brand-purple font-bold">
          ← Back to track
        </button>
      </div>
    )
