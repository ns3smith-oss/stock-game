...
const PROGRESS_KEY = 'stockly_leveler_progress'
...
export default function LevelerTrackPage() {
  ...
  const totalLessons = LEVELER_UNITS.reduce((acc, u) => acc + u.lessons.length, 0)
  ...
  function handleLesson(lessonId: string, state: string) {
    ...
    haptics.tap()
    router.push(`/learn/leveler/lesson/${lessonId}`)
  }
    ...
    <div className="max-w-sm mx-auto px-6 py-6 pb-20 md:max-w-2xl">
      ...
      <div className="bg-brand-purple/20 border-2 border-brand-purple rounded-3xl p-5 mb-6 animate-slideUp">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h1 className="text-xl font-black text-brand-white">Leveler Track</h1>
            <p className="text-brand-muted text-xs">Charts, risk management, and building a real strategy.</p>
          </div>
        </div>
      ...
      {completedCount === totalLessons && totalLessons > 0 && (
        <div className="bg-brand-green/20 border-2 border-brand-green rounded-3xl p-6 text-center animate-levelBurst">
          ...
          <h2 className="text-brand-white font-black text-xl mb-2">Leveler Track Complete!</h2>
          <p className="text-brand-muted text-sm">Charts, risk management, market cycles — you've leveled up for real. Wealth Building awaits.</p>
        </div>
      )}
