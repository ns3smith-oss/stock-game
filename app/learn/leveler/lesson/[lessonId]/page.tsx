...
const XP_KEY = 'stockly_leveler_xp'
...
export default function LevelerLessonPage() {
  ...
  useEffect(() => {
    ...
    for (const unit of LEVELER_UNITS) {
  ...
  function handleComplete(xp: number) {
    ...
    updateUser({ onboardingStep: '/learn/leveler' })
    router.push('/learn/leveler')
  }
  ...
  if (notFound) {
    return (
      <div className="max-w-sm mx-auto px-6 py-20 text-center">
        ...
        <button onClick={() => router.push('/learn/leveler')} className="text-brand-purple font-bold">
          ← Back to track
        </button>
      </div>
    )
