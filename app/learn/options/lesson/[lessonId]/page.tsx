...
const XP_KEY = 'stockly_options_xp'
...
export default function OptionsLessonPage() {
  ...
  useEffect(() => {
    ...
    for (const unit of OPTIONS_UNITS) {
  ...
  function handleComplete(xp: number) {
    ...
    updateUser({ onboardingStep: '/learn/options' })
  ...
  if (notFound) {
    return (
      <div className="max-w-sm mx-auto px-6 py-20 text-center">
        ...
        <button onClick={() => router.push('/learn/options')} className="text-brand-purple font-bold">
          ← Back to track
        </button>
      </div>
    )
