...
const PROGRESS_KEY = 'stockly_options_progress'
...
export default function OptionsTrackPage() {
  ...
  const totalLessons = OPTIONS_UNITS.reduce((acc, u) => acc + u.lessons.length, 0)
  ...
  function handleLesson(lessonId: string, state: string) {
    ...
    haptics.tap()
    router.push(`/learn/options/lesson/${lessonId}`)
  }
  ...
  return (
    <div className="max-w-sm mx-auto px-6 py-6 pb-20 md:max-w-2xl">
      ...
      {/* Track hero */}
      <div className="bg-blue-500/10 border-2 border-blue-500 rounded-3xl p-5 mb-6 animate-slideUp">
        <div className="flex items-center gap-3 mb-3">
          <div className="text-4xl">📊</div>
          <div>
            <h1 className="text-xl font-black text-brand-white">Options Trading</h1>
            <p className="text-brand-muted text-xs">Contracts, Greeks, entries, exits, and real strategy.</p>
          </div>
        </div>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-4 p-4 rounded-2xl border-2 text-left transition-all active:scale-95 ${
                      isComplete
                        ...
                        : 'bg-brand-surface border-blue-500/60 shadow-lg shadow-blue-500/10'
                    }`}
                  >
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0 ${
                      isComplete ? 'bg-brand-green/20' : isLocked ? 'bg-white/5' : 'bg-blue-500/10'
                    }`}>
                    ...
                    {!isLocked && (
                      <span className={`text-sm font-bold flex-shrink-0 ${isComplete ? 'text-brand-green' : 'text-blue-400'}`}>
                        {isComplete ? '★' : '→'}
                      </span>
                    )}
                  </button>
      ...
      {completedCount === totalLessons && totalLessons > 0 && (
        <div className="bg-blue-500/10 border-2 border-blue-500 rounded-3xl p-6 text-center animate-levelBurst">
          <div className="text-5xl mb-3">📊</div>
          <h2 className="text-brand-white font-black text-xl mb-2">Options Track Complete!</h2>
          <p className="text-brand-muted text-sm">Contracts, Greeks, directional plays, trend trading, earnings, and the Wheel. You have a complete options education.</p>
        </div>
      )}
