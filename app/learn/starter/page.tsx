...
export default function StarterTrackPage() {
  ...
  const totalLessons = STARTER_UNITS.reduce((acc, u) => acc + u.lessons.length, 0)
  ...
  function handleLesson(lessonId: string, state: string) {
    ...
    haptics.tap()
    router.push(`/learn/starter/lesson/${lessonId}`)
  }
  ...
  return (
    <div className="max-w-sm mx-auto px-6 py-6 pb-20">
      ...
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => router.push('/')} className="text-brand-muted text-sm active:scale-95">
          ✕
        </button>
      ...
      <div className="bg-brand-purple/20 border-2 border-brand-purple rounded-3xl p-5 mb-6 animate-slideUp">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h1 className="text-xl font-black text-brand-white">Starter Track</h1>
            <p className="text-brand-muted text-xs">Start from zero. No experience needed.</p>
          </div>
        </div>
      ...
      {completedCount === totalLessons && totalLessons > 0 && (
        <div className="bg-brand-green/20 border-2 border-brand-green rounded-3xl p-6 text-center animate-levelBurst">
          ...
          <h2 className="text-brand-white font-black text-xl mb-2">Track Complete!</h2>
          <p className="text-brand-muted text-sm">You've finished the Starter track. You're ready to invest.</p>
        </div>
      )}
